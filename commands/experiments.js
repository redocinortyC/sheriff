	...
	data: new SlashCommandBuilder()
		...
		.addSubcommand(subcommand =>
			subcommand
				...
				.addStringOption(option => option.setName('username').setDescription('New username for the bot'))
				.addStringOption(option =>
					option.setName('activity-type')
						.setDescription('Sets status to "Watching/Listening to/Competing in ..."')
						.addChoice('Watching', 'WATCHING')
						.addChoice('Listening to', 'LISTENING')
						.addChoice('Competing in', 'COMPETING'))
				.addStringOption(option => option.setName('activity').setDescription('New activity for the bot'))),
	...
	async execute(interaction) {
		...
		*/
		...
		if (interaction.options.getSubcommand() === 'set') {
			...
			if (username && interaction.member.permissions.has(Permissions.FLAGS.MANAGE_NICKNAMES)){
				client.user.setUsername(username);
			} else {
				await interaction.reply({ content: ':space_invader: Oops! An error occured.', ephemeral: true });
			}

			if (activityType && activity && interaction.member.permissions.has(Permissions.FLAGS.MANAGE_NICKNAMES)){
